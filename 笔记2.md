...
1. Client Hello  发送client_random 支持的算法等
2. Server Hello  发送server_random 支持的算法等
3. 服务器证书 信任建⽴ 客户端验证证书 (有的服务器也要验证客户端的证书 比如银行的U盾)
4. Pre-master Secret  客户端通过公钥加密Pre-master Secret 发送到服务器 然后根据client_random  server_random Pre-master Secret  通过一定的算法 生成加密通信的key
5. 客户端通知：将使⽤加密通信 
6. 客户端发送：Finished 
7. 服务器通知：将使⽤加密通信 
8. 服务器发送：Finished
...
1.Server 通过binder驱动 将自己能提供的服务注册到serviceManager,serviceManager会建立一个映射表
...
1.开机键引导芯片加载BootLoader到内存 拉起LinuxOS

​	linux内核启动完毕后  寻找init.rc文件并启动init进程

2.init进程 

​	创建和挂载启动所需要的目录,并对属性服务进行初始化和启动, 启动守护

​	然后去解析init.rc配置文件,启动Zygote进程
...
​	通过registerZygoteSocket方法 建立一个服务器Socket,等待AMS来请求Zygote创建新的应用程序进程,同时进行预加载类和资源

​	通过runSelectLoop方法等待AMS的请求来创建新的应用进程

​	启动SystemServer进程,通过fork方式启动,进程名为：system_server，启动的类名为：		com.android.server.SystemServer 
