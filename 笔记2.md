...
签名的内容:证书信息+证书公钥+证书签名算法 做hash,然后用签发机构的私钥对这个hash做一次加密(签名),

验证证书:

1.对证书信息使用相同的hash算法得到h1,然后用签发机构的公钥对服务器加密后的签名做一次加密运算得到h2,如果h1 == h2 则认为证书安全
...
4. Pre-master Secret  客户端通过证书公钥加密Pre-master Secret 发送到服务器 然后根据client_random  server_random Pre-master Secret  通过一定的算法(PRF(Pseudo-Random Function)) 生成加密通信的key
5. 客户端通知：将使⽤加密通信 
6. 客户端发送：Finished 
7. 服务器通知：将使⽤加密通信 
8. 服务器发送：Finished
...
EXACTLY 用父view给的size,

AT_MOST比对自己想要的和父view给你的size比对 超出就用父view 没超出用自己

UNSPECIFIED 无限制 用自己的size
...
attach方法最终执行到ActivityThread的handleLauncherActivity()方法
...
###### 直接内存 不是JVM规范中定义的内存区域,但是也经常使用 也会OOM,比如NIO的Buffer分配的内存
...
​		对象头: MarkWord 类型指针(对象指向它的类原数据的指针,部分虚拟机通过这个判断是那个类的实例) 数组长度(对象是数组时)

​		具体数据:定义的各种类型的字段内容

​		对齐填充:保证对象的大小是8字节的倍数,这个是JVM要求的
...
4)运行时常量池,运行时常量池是方法区的一部分,class文件中的常量池会加载到这里

静态对象一般在堆中

对象的访问定位: 句柄或者直接指针两种方式 
...
GC root :Garbage Collector root :Static对象,线程持有的对象,本地方法的引用 虚拟机栈(栈帧中的本地变量表)中的引用对象,方法区中常量引用的对象
...
新生代和老年代用的算法不同 分代收集算法: 新生代和老年代用不同的回收算法

新生代: Eden和Survivor区域为8:1,使用标记复制算法

老年代:标记-整理算法或标记-清除算法

finalize方法建议不要使用,可以用try finally去做善后工作,finalize并不一定执行

###### 回收方法区

没有被引用的常量会被系统清理出常量池,类,接口,方法,字段的符号引用等也类似

但是类的回收条件比较苛刻:

1.类的所有实例被回收,也就是堆中不存在该类的实例
...
主线程的 MessageQueue没有消息时，便阻塞在loop的 queue.next()中的 nativePollOnce()方法里，此时主线程会释放CPU资源进入休眠状态，直到下个消息到达或者有事务发生，通过往pipe管道写端写入数据来唤醒主线程工作。这里采用的epoll机制，是一种IO多路复用机制，可以同时监控多个描述符，当某个描述符就绪(读或写就绪)，则立刻通知相应程序进行读或写操作，本质同步I/O，即读写是阻塞的。 所以说，主线程大多数时候都是处于休眠状态，并不会消耗大量CPU资源。 Gityuan–Handler(Native层)
...
3.Application加载问题,每个module定义自己的一个初始化类,继承一个统一的抽象类,里面写好onCreate,attach等方法,,然后每个module的初始化类中,写好自己的初始化逻辑.在主module中,将这些类通过反射的方式初始化,然后在对应的生命周期方法中,调用这个类的对应方法,就完成了Application的初始化,同时自己的module中也要注册自己的这个类,因为要自己完成自己的初始化工作
...
volatile 适用于读多写少的场景,每个线程要操作volatile修饰的变量都需要从内存拷贝副本,当线程操作变量副本并写回内存后,会通过`CPU总线嗅探机制`告诉其他线程被修改了,副本已失效,需要重新从主内存中读取
禁止指令重排:代码写的是 a= 1; b = true;但是实际可能是 b =true; a = 1; 多线程情况下 可能会对执行结果造成影响
实现方式:内存屏障 四个指令 LoadLoad   LoadStore StoreLoad StoreStore  StoreLoad是全能型屏障,但是开销大
示例 :确保load1的读操作,在load2及所有后续操作之前
    load1; 
    LoadLoad;
    Load2;
...
isInterrupt()测试当前线程是否已经中断 并重置标志位

isInterrupted() 测试当前线程是否已经中断 不重置标志位 可在线程中用这个方法判断是否应该退出线程
...
shrinkResources true  配合混淆

图片使用webp,矢量图,只使用一套图等
...
horizontal_constraint_weight 水平方向的weight 像linearlayout一样

Baseline_toBaselineOf 对齐文字的baseline

0dp =match_constraint

圆形定位

constraintCircleAngle="45"

constraintCircleRadius="150dp"

constraintCircle="@+id/sun"

限定约束

layout_constrainedWidth="true"

//水平方向偏向 同样还有垂直偏向 范围0.0到1.0
...
//垂直方向约束链的Style 约束链:两个控件互相约束 连接在一起

constraintVertical_chainStyle="packed"

//设定控件宽高比 使用这个属性宽高至少一个是0dp

constraintDimensionRatio="2:1" 宽高比 或者H,2:1

constraintDimensionRatio="W,2:1" 高宽比 一般不用

W,H 是被约束的 也就是剩下的值是确定的
...
constraintGuide_percent="0.24" 百分比切割 可以负数

constraintGuide_begin="150dp" //辅助线从左到右边的位置切割

</androidx.constraintLayout.widget.Guideline>

//使用group对子view显示隐藏统一处理

<androidx.constraintLayout.widget.Group>

constraint_referenced_ids="button5,button6"

</androidx.constraintLayout.widget.Group>

//使用layer统一做动画 但是没有过程 直接到结果
...
constraint_referenced_ids="button5,button6" //可能会被混淆导致出bug 要处理

</androidx.constraintLayout.widget.Barrier>

Math.hypot 计算斜边

可以在ConstraintHelper的 updatePostLayout中 做动画,继承后 放在布局中 传入id 就会自动执行对应的动画

PlaceHolder 占位view的 传入view的id 会到viewholder中

view.setonclickListener{

viewholder.setContentId(it.id) 

}

//使用ContraintSet 调整布局位置
...
clone(context,R.layout.activity_constraint_end)
...
val constrainSet1 = ConstrainSet()
//加动画
TransitionManager.beginDelayedTransition(constraintLayout)
//设置的view的id,哪条边,被约束的id,和哪条边约束
constraintSet1.connect(R.id.twitter,ContrainSet.Start,ConstraintSet.PARENT_ID,ConstraintSet.START)
constraintSet1.applyTo(constraintLayout)
```



//自定义约束VirtualLayout helper 目的是解耦
...
```kotlin
 class Linear(context:Context,attrs:AttributeSet):VirtualLayout(context,attrs){
	...
	override fun updatePreLayout(container:ConstraintLayout){
        ...
        constraintSet.clone(container)//复制已经有的约束
...
//Flow helper  帮助实现LinearlLayout的布局

flow_verticalGap="8dp" 间距
...
1.JVM基于栈 DVM基于寄存器(register)

2.DVM加载的是dex文件 JVM是class文件

3.DVM通过Zygote进行Class Preloading,Zygote会完成续集你的初始化工作

#### TCP 粘包如何解决
每个文件的最后附加一些特殊的字节,或者控制每个报文只包含一个文件的数据

#### 锁
乐观锁 悲观锁
synchronized lock 都是悲观锁,适合写操作多的场景
synchronized 在对象头里 通过monitor实现线程同步,monitor依赖底层的Mutex Lock(互斥锁)来实现线程同步 未优化前是重量级锁
CAS 是乐观锁 compare and swap java中的Atomic类就是CAS实现的 适合读操作多的场景
自旋锁:拿不到锁会一直自旋,也是CAS实现的
锁的状态
无锁:没有对资源进行锁定,多个线程都可以访问,但是只有一个能成功
偏向锁:指的是一段同步代码被一个线程所访问,线程会自动获得锁,降低获取锁的代价,线程不会主动释放偏向锁,只有其他线程来竞争的时候才会释放,偏向锁在置换线程id时只需要一次CAS原子指令即可
轻量级锁:指的是当锁是偏向锁时,被另外的线程访问,偏向锁就会升级为轻量级锁,其他线程会通过自旋的形式尝试获得锁
重量级锁:自旋超过一定次数(一般10次),或者有一个正在自旋 又来个线程 就会升级到重量级锁,等待锁的进程会进入阻塞状态,需要被唤醒

综上，偏向锁通过对比Mark Word解决加锁问题，避免执行CAS操作。而轻量级锁是通过用CAS操作和自旋来解决加锁问题，避免线程阻塞和唤醒而影响性能。重量级锁是将除了拥有锁的线程以外的线程都阻塞。

公平锁 不公平锁
公平锁:多个线程按照顺序获取锁,等待的线程不会饿死.但是吞吐效率比非公平锁低,等待队列里除了第一个现场都会阻塞,CPU唤醒阻塞线程的开销比非公平锁大
非公平锁:多个线程加锁时直接尝试获取锁,获取不到才会去队列尾部等待,如果正好锁可用,就无需等待直接获得锁,所以非公平锁可能会有后申请锁的线程先拿到锁,可以减少唤醒线程的开销,吞吐效率高,因为线程有几率直接获得锁,不进入阻塞状态,CPU不需要唤醒所有线程,但是等待队列中的线程可能会饿死,或者等待很久才能获得锁
ReentrantLock 默认是非公平锁 可以设置改成公平锁

可重入锁 不可重入锁
可重入锁又叫递归锁 就是同一个锁可以lock多次 比如synchronized ReentrantLock 都是可重入锁,可以避免死锁
不可重入锁必须先释放锁,但是锁被持有无法释放,所以会造成死锁
NonReentrantLock 不可重入锁
AbstractQueuedSynchronizer中有 status字段 记录锁的次数 不可重入锁只有status -1 == 0时,才会真的释放锁

独享锁 共享锁
ReentrantReadWriteLock 读锁就是共享锁 写锁是独享锁 



#### MotionLayout

TransitionManager 在布局参数改变之前调用beginDelayedTransition 就可以有过渡动画,传入的参数是执行动画的view的父容器

过渡动画的大致流程

1. 两个场景 一个开始场景 一个结束场景,记录场景上的控件的各种参数(宽高位置等)
2. 有了参数,创建出动画对象
3. 播放动画

具体逻辑,在requestlayout之前开始调用TransitionManager .beginDelayTransition(v.parent);会捕获当前布局的布局参数,并设置监听(viewTreeObservable.addOnPreDrawListener) 监听绘制之前的监听,就可以拿到布局修改后的参数,然后创建并执行动画

另外一种调用方式 TransitionManager .go(scene) 

通过

val startScene = Scene.getSceneForLayout(rootView,R.layout.go_start,this)



val endScene = Scene.getSceneForLayout(rootView,R.layout.go_end,this)

创建出来scene对象,然后调用go函数,分别传入scene就可以了

最后需要重新绑定数据 也就是对view的数据的绑定需 要重新执行一次



可以通过ConstraintSet()配合TransitionManager.beginDelayedTransition(root) root必须为ConstraintLayout

```kotlin
TransitionManager.beginDelayedTransition(root)
val set = ConstraintSet()
if(isToggle){
    set.clone(this,R.layout.activity_constraint_start)
}else {
    set.clone(this,R.layout.activity_constraint_end)
}
set.applyTo(root)
isToogle = !isToogle
```

 缺点:布局重复只是位置不同 约束条件不同,而且还要放到layout文件夹,不易于维护



MotionLayout 设置一个layoutDescription 指定一个xml文件就可以

xml文件根节点是MotionScene
...
<MotionScene>
    <Transition 
        ...
        <--app:constraintSetEnd="@layout/activity_constraint_end"-->
           >
            <-第二种方式引用ConstraintSet指定的id->
        <--app:constraintSetStart="@id/start"-->
        <--app:constraintSetEnd="@id/end">-->
        	<--给指定id控件设置点击事件 -->
    	<Onclick 
            app:targetId="@+id/fab_favorite"
