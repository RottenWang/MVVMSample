...
线程池流程: 添加runnable时,先判断coolPoolSize ,如果小于 启动新线程去做任务,如果大于 判断队列是否满,没满 放队列去等待被调度,如果满了,判断是否大于maximumPoolSize,不大于则创建新的线程去执行任务(这里创建的线程如果空闲时间超出,会被回收),大于就走对应的超出策略
...
原理:在编译器解析@Route注解的类,把path和activity.class文件映射保存起来,通过build方法保存路径,然后通过navigation 匹配路径
