...
非公平锁:多个线程加锁时直接尝试获取锁,获取不到才会去队列尾部等待,如果正好锁可用,就无需等待直接获得锁,所以非公平锁可能会有后申请锁的现场先拿到锁,可以减少唤醒线程的开销,吞吐效率高,因为线程有几率直接获得锁,不进入阻塞状态,CPU不需要唤醒所有线程,但是等待队列中的线程可能会饿死,或者等待很久才能获得锁
